        ...
        for (int offset = 0; offset < 192; offset += (int)vl4) {
...

            {
                ...
                if (start < avl) {
                    ...
                    vfloat32m2_t sum = __riscv_vfadd_vv_f32m2(xa, xb, vlc);
                    vfloat32m2_t mix = __riscv_vfmul_vv_f32m2(xa, xc, vlc);
                    vfloat32m2_t centered = __riscv_vfsub_vv_f32m2(sum, xx, vlc);
                    vfloat32m2_t scaled = __riscv_vfmul_vv_f32m2(centered, xc, vlc);
                    vfloat32m2_t fused = __riscv_vfadd_vv_f32m2(scaled, mix, vlc);
                    fused = __riscv_vfadd_vv_f32m2(fused, sum, vlc);
                    vfloat32m2_t out = __riscv_vfmacc_vf_f32m2(xb, 0.5f, fused, vlc);
