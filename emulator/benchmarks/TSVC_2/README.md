...
`src/tsvc.c` can be split into individual loop sources under `src/loops/` by running:

```
python scripts/generate_loops.py
```

Each generated file contains the loop itself plus a small wrapper that prepares the arguments expected by that loop. You can build a single-loop binary (with the shared data initialisation, checksum and timing) with:

```
make COMPILER=llvm_rvv loop-s171
```

The resulting binaries are written to `tsvc2_build/bin/<compiler>/loops/<loop>`. Use `make COMPILER=... loops` to build them all.
