...
#ifndef TSVC_DEFAULT_FILTER
#define TSVC_DEFAULT_FILTER ""
#endif
...
static void parse_selection(struct selection_t *sel, const char *list) {
    sel->count = 0;
    if (!list || !*list) {
        return;
    }
    ...
    if (len >= sizeof(sel->buffer)) {
        len = sizeof(sel->buffer) - 1;
    }

    memcpy(sel->buffer, list, len);
    sel->buffer[len] = '\0';

    char *token = strtok(sel->buffer, ",");
    while (token && sel->count < (sizeof(sel->items) / sizeof(sel->items[0]))) {
        token = trim_token(token);
        if (*token) {
            sel->items[sel->count++] = token;
        }
        token = strtok(NULL, ",");
    }
}

static int should_run(const struct selection_t *sel, const char *name) {
    if (sel->count == 0) {
        return 1;
    }

    for (size_t i = 0; i < sel->count; i++) {
        if (strcmp(sel->items[i], name) == 0) {
            return 1;
        }
    }
...
int main(int argc, char ** argv){
    ...
    if (argc > 1 && argv[1] && argv[1][0] != '\0') {
        filter = argv[1];
    } else if (TSVC_DEFAULT_FILTER[0] != '\0') {
        filter = TSVC_DEFAULT_FILTER;
    }

    struct selection_t selection = {0};
    parse_selection(&selection, filter);

    printf("Loop \tTime(sec) \tChecksum\n");

#define RUN(loop, arg_expr) do {                   \
        if (should_run(&selection, #loop)) {       \
            time_function(&loop, arg_expr);        \
        }                                          \
    } while (0)
    ...
    RUN(s1119, NULL);
    RUN(s121, NULL);
    RUN(s122, (&(struct{int a;int b;}){n1, n3}));
    RUN(s123, NULL);
    RUN(s124, NULL);
    RUN(s125, NULL);
    RUN(s126, NULL);
    ...
    RUN(s162, &n1);
    RUN(s171, &n1);
    RUN(s172, (&(struct{int a;int b;}){n1, n3}));
    RUN(s173, NULL);
    RUN(s174, (&(struct{int a;}){LEN_1D/2}));
    RUN(s175, &n1);
    RUN(s176, NULL);
    RUN(s211, NULL);
    RUN(s212, NULL);
    ...
    RUN(s235, NULL);
    RUN(s241, NULL);
    RUN(s242, (&(struct{real_t a;real_t b;}){s1, s2}));
    RUN(s243, NULL);
    RUN(s244, NULL);
    RUN(s1244, NULL);
    RUN(s2244, NULL);
    ...
    RUN(s482, NULL);
    RUN(s491, ip);
    RUN(s4112, (&(struct{int*a;real_t b;}){ip, s1}));
    RUN(s4113, ip);
    RUN(s4114, (&(struct{int*a;int b;}){ip, n1}));
    RUN(s4115, ip);
    RUN(s4116, (&(struct{int * a; int b; int c;}){ip, LEN_2D/2, n1}));
    RUN(s4117, NULL);
    RUN(s4121, NULL);
    RUN(va, NULL);
    RUN(vag, ip);
